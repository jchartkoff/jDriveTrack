...
public class WorldWindBulkDownloader extends JPanel {
	private static final long serialVersionUID = 6515741900406849101L;
	...
	public WorldWindBulkDownloader(WorldWindow wwd) {
		this.wwd = wwd;
		...
		for (Layer layer : this.wwd.getModel().getLayers()) {
			if (layer instanceof BulkRetrievable)
				this.retrievables.add(new BulkRetrievablePanel(
						(BulkRetrievable) layer));
		}
		// Elevation models
		CompoundElevationModel cem = (CompoundElevationModel) wwd.getModel()
		...
		for (ElevationModel elevationModel : cem.getElevationModels()) {
			if (elevationModel instanceof BulkRetrievable)
				this.retrievables.add(new BulkRetrievablePanel(
						(BulkRetrievable) elevationModel));
		}
		...
		for (BulkRetrievablePanel panel : this.retrievables) {
			if (panel.selectCheckBox.isSelected()) {
				...
				BulkRetrievalThread thread = retrievable.makeLocal(
						this.currentSector, 0, this.cache,
	...
	protected void initComponents() {
		...
		selectButton
				.setToolTipText("Press Select then press and drag button 1 on globe");
		...
		retrievablesPanel.setLayout(new BoxLayout(retrievablesPanel,
				BoxLayout.Y_AXIS));
		...
		startPanel.setBorder(BorderFactory.createEmptyBorder(border, border,
				border, border));
	...
	public class BulkRetrievablePanel extends JPanel {
		private static final long serialVersionUID = -3160597989635417256L;
		...
		protected void doUpdateDescription(final Sector sector) {
			if (sector != null) {
				try {
					long size = retrievable.getEstimatedMissingDataSize(sector,
							0, cache);
					final String formattedSize = WorldWindBulkDownloader
							.makeSizeDescription(size);
	...
	public class DownloadMonitorPanel extends JPanel {
		private static final long serialVersionUID = 1110754743656600640L;
		...
		protected void updateStatus() {
			...
			text += " ("
					+ WorldWindBulkDownloader.makeSizeDescription(this.progress
							.getCurrentSize())
					+ " / "
					+ WorldWindBulkDownloader.makeSizeDescription(this.progress
							.getTotalSize()) + ")";
			...
			if (this.progress.getTotalCount() > 0)
				percent = (int) ((float) this.progress.getCurrentCount()
						/ this.progress.getTotalCount() * 100f);
			this.progressBar.setValue(Math.min(percent, 100));
			// Update tooltip
			String tooltip = WorldWindBulkDownloader
					.makeSectorDescription(this.thread.getSector());
		...
		protected String makeProgressDescription() {
			String text = "";
			if (this.progress.getTotalCount() > 0) {
				int percent = (int) ((double) this.progress.getCurrentCount()
						/ this.progress.getTotalCount() * 100d);
				text = percent + "% of ";
