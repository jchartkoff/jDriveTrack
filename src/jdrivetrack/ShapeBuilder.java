...
public class ShapeBuilder implements Shape {
	...
	public ShapeBuilder(Point.Double[] pointArray ) {
		generalPath = generalPath(pointArray);
	}
	
    private GeneralPath generalPath(Point.Double[] pointArray) {
    	GeneralPath generalPath = new GeneralPath();
    	generalPath.moveTo(pointArray[0].x, pointArray[0].y);
    	for (int i = 1; i < pointArray.length; i++) {
	        Point.Double n = pointArray[i]; 
	    	generalPath.lineTo(n.x, n.y);
	    }
		return generalPath;
    }    
