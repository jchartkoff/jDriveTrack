...
import jdrivetrack.TileCache;

public class Tile {

    public static final BufferedImage LOADING_IMAGE = loadImage("images" + File.separator + "hourglass.png");

    public static final BufferedImage ERROR_IMAGE = loadImage("images" + File.separator + "error.png");
    ...
    private static BufferedImage loadImage(String path) {
        try {
            return ImageIO.read(OpenStreetMapPanel.class.getResourceAsStream(path));
        } catch (Exception ex) {
            ex.printStackTrace();
            return null;
        }
    ...
    public void loadPlaceholderFromCache(TileCache cache) {
        ...
        for (int zoomDiff = 1; zoomDiff < 5; zoomDiff++) {
            ...
            if (zoomDiff < 3 && zoomHigh <= source.getMaxZoom()) {
