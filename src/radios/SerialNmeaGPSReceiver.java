...
import interfaces.GPSInterface.FAAMode;
import interfaces.GPSInterface.FixQuality;
import jdrivetrack.Coordinates;
...
public class SerialNmeaGPSReceiver implements GPSInterface {
				...
				if (!checksum(msg)) {
					...
					if (a[0].equals("$GPRMC")) {
						...
						if (a[1].length() != 0 && a[9].length() != 0) {
							int seconds = Integer.parseInt(a[1].substring(4, 6));
							...
							date.set(Calendar.MILLISECOND, seconds * 1000);
						}
    ...
    private FixQuality getFixQuality(String fq) {
        switch (fq) {
	        ...
	        default: return  FixQuality.ACQUIRING;
        }
	}
