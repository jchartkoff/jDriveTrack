...
public class SerialNmeaGPSReceiver implements GPSInterface {
	...
	private void nmeaDecoder(String message) {
		try {
			...
			pcs.firePropertyChange(GPSInterface.NMEA_DATA, null, msg);
				...
				if (!checksum(msg)) {
					...
					if (a[0].equals("$GPRMC")) {
						...
						if (a[1].length() != 0 && a[9].length() != 0) {
							...
							pcs.firePropertyChange(VALID_TIME, null, date.getTimeInMillis());
						}
						...
						if (a[8].length() != 0) {
							if (speedMadeGoodMPH > 5.0) {
								...
								pcs.firePropertyChange(COURSE_MADE_GOOD_TRUE, null, courseMadeGoodTrue);
								pcs.firePropertyChange(COURSE_MADE_GOOD_MAGNETIC, null, courseMadeGoodMagnetic);
							}
						}
						if (a.length > 12) {
							if (a[12].length() != 0) {
						...
						if (a[2].length() != 0 && a[4].length() != 0) {
							...
							pcs.firePropertyChange(VALID_WAYPOINT, null, aprsPosition);
						}
					}
				}
			}
	...
	public String getUTMCoordinates() {
		return Coordinates.convertLonLatToUTM(position);
	}
    ...
    private FixQuality getFixQuality(String fq) {
        switch (fq) {
	        ...
	        default: return  FixQuality.GPS_DOWN;
        }
	}
    ...
    public void zeroize() {
    	...
    	validTrueRdfHeading = false;
    	speedMadeGoodMPH = -1;
    	speedMadeGoodKnots = -1;
    	speedMadeGoodKPH = -1;
    	altitude = -1;
    	altitudeOverEllipsoid = -1;
    	satellitesInView = -1;
    	...
    	pcs.firePropertyChange("VALID_FIX", null, validFix);
    	pcs.firePropertyChange("FAA_MODE", null, faaMode);
    	pcs.firePropertyChange("CRC_ERROR", null, "");
    	pcs.firePropertyChange("NMEA_DATA", null, "");
    }
